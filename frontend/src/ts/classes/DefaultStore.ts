...
import {encodeHTML, dateToChineseString} from "@/ts/utils/htmlApi";
import {
...
export class DefaultStore extends VuexModule {
  ...
  get userName(): (id: number) => string {
    return (id: number): string => {
      const user = this.allUsersDict[id];
  ...
  public setStateFromWS(state: SetStateFromWS) {
    ...
    // 验证并过滤房间数据，确保数据一致性
    const validatedRoomsDict: RoomDictModel = {};
    Object.entries(state.roomsDict).forEach(([roomId, room]) => {
      const id = parseInt(roomId, 10);
      // 验证房间数据的有效性
      if (id && id > 0 && room && room.id === id) {
        validatedRoomsDict[id] = room;
      } else {
        logger.warn(`Invalid room data detected and filtered: roomId=${roomId}, room:`, room)();
      }
    });

    this.roomsDict = validatedRoomsDict;
  ...
  public setStateFromStorage(setRooms: SetStateFromStorage) {
    ...
    // 验证并过滤从本地存储恢复的房间数据
    const validatedRoomsDict: RoomDictModel = {};
    Object.entries(setRooms.roomsDict).forEach(([roomId, room]) => {
      const id = parseInt(roomId, 10);
      // 验证房间数据的有效性
      if (id && id > 0 && room && room.id === id) {
        validatedRoomsDict[id] = room;
      } else {
        logger.warn(`Invalid room data from storage detected and filtered: roomId=${roomId}, room:`, room)();
      }
    });
