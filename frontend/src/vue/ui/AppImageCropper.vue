    ...
    <div class="imgHolder">
      <section class="cropper-area">
        ...
        <div v-if="srcImg" class="actions">
          <i class="icon-zoom-in" title="放大 (鼠标滚轮)" @click="cropper.relativeZoom(0.2)"/>
      <i class="icon-zoom-out" title="缩小 (鼠标滚轮)" @click="cropper.relativeZoom(-0.2)"/>
      <i class="icon-rotate" title="旋转" @click="cropper.rotate(90)"/>
      <i class="icon-resize-horizontal" title="水平翻转" @click="cropper.scaleX(scaleX = scaleX * -1)"/>
      <i class="icon-resize-vertical" title="垂直翻转" @click="cropper.scaleY(scaleY = scaleY * -1)"/>
        </div>
      </section>
      <video
        v-show="showVideo"
